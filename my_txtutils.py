...
def find_book(index, fileranges):
    return next(
        book["name"] for book in fileranges if (book["start"] <= index < book["end"]))


def find_book_index(index, fileranges):
    return next(
        i for i, book in enumerate(fileranges) if (book["start"] <= index < book["end"]))


def print_learning_learned_comparison(X, Y, losses, fileranges, batch_loss, batch_accuracy, epoch_size, index, epoch):
...
def read_data_files(globby, validation=True):
    ...
    codetext = []
    fileranges = []
    filelist = glob.glob(globby, recursive=True)
    for filename in filelist:
        file = open(filename, "r")
        print("Loading file " + filename)
        start = len(codetext)
        codetext.extend(encode_text(file.read()))
        end = len(codetext)
        fileranges.append({"start": start, "end": end, "name": filename.rsplit("/", 1)[-1]})
        file.close()
    ...
    validation_len = 0
    nb_books1 = 0
    for book in reversed(fileranges):
    ...
    if nb_books == 0 or not validation:
        ...
        cutoff = fileranges[-nb_books]["start"]
    valitext = codetext[cutoff:]
    codetext = codetext[:cutoff]
    return codetext, valitext, fileranges


def print_data_stats(datalen, valilen, epoch_size):
    datalen_mb = datalen / 1024.0 / 1024.0
    valilen_kb = valilen / 1024.0
    print("Training text size is {:.2f}MB with {:.2f}KB set aside for validation.".format(datalen_mb, valilen_kb) + " There will be {} batches per epoch".format(epoch_size))


def print_validation_header(validation_start, fileranges):
    bookindex = find_book_index(validation_start, fileranges)
    books = ''
    for i in range(bookindex, len(fileranges)):
        books += fileranges[i]["name"]
